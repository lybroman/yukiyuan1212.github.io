(function(){
	...
	Center.prototype = {
		...
		activate : function () {
			...
			var zh_sh = $("span[data-src='Asia/Shanghai']").text()
			if(zh_sh != '')
			{
				var zh_sh_m = moment.tz(zh_sh.replace(/T/, ' '), 'Asia/Shanghai')
				var selected_city = zh_sh_m.clone().tz(this.name)
				$labelTime.text(selected_city.format());
			}
			else
			{
				$labelTime.text(moment().tz(this.name).format());
			}
	...
	*/
	...
	var city_lookup = $.map(cities, function(dt) { return {value:dt, data:{ category: 'Region/City'}}; })
